...
public class GiaiDien extends JFrame implements ActionListener,MouseListener{
	...
	public GiaiDien(DanhSachNhanVien dao) {
		setTitle("^0^");
		setSize(700,450);
	...
	private void themActions() {
		try {
			...
			String isNu = phai ? "nu" : "nam";
			int tuoi = Integer.parseInt(txtTuoi.getText());
			double luong= Double.parseDouble(txtLuong.getText());
			NhanVien nv = new NhanVien(maNV, ho, ten, isNu, tuoi, luong);
			if(dao.themNhanVien(nv)) {
				String[] row= {maNV,ho,ten,isNu,Integer.toString(tuoi),luong+""};
			...
			if(new File(FILENAME).exists()) {
				...
				while(br.ready()){
					...
					if(line !=null&& !line.trim().equals("")) {
						String[]a= line.split(";");
						NhanVien nv= new NhanVien(a[0],a[1],a[2],a[3], Integer.parseInt(a[4]),Double.parseDouble(a[5]));
						dao.themNhanVien(nv);
						tableModel.addRow(a);
					}
				}
	...
	new GiaiDien(dao).setVisible(true);
	}
}
